...
describe("Editor.vue", () => {
  ...
  it("renders the correct HTML structure set editor true", () => {
    ...
    const expectedHtml = `<v-mainid="editor-component"><v-rowclass="main-rowma-0pa-0"><v-colclass="main-colma-0pa-0"><v-cardclass="main-cardmr-16pb-2"><v-card-titleclass="task-intro"><v-imgclass="task-icon"style="display:none;"></v-img><divclass="task-label"></div><v-spacer></v-spacer><div><v-dividervertical=""inset=""></v-divider><v-btnstyle="float:right;"id="exec"text=""tile=""disabled="true"><divclass="exec-title"></div><divclass="exec-icon"></div></v-btn></div></v-card-title><v-divider></v-divider><v-listvalue="GeneralProperties"><v-list-groupvalue="GeneralProperties"><v-list-item><formquestions=""></form></v-list-item></v-list-group></v-list></v-card></v-col></v-row><v-dividerclass="mr-16"></v-divider><v-rowstyle="height:4rem;"class="mr-16"sm="auto"><v-colclass="bottom-buttons-col"style="display:flex;align-items:center;"><v-btnid="save"disabled="true">Save</v-btn></v-col></v-row></v-main>`;
    expect(wrapper.html().replace(/\s/g, "")).to.equal(expectedHtml);
    expect(wrapper.exists()).to.be.true;
  });
