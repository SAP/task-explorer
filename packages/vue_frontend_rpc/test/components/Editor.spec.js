...
describe("Editor.vue", () => {
  ...
  it("correctly prepares the task", () => {
    ...
    // Call the prepareTask method with the sample task
    wrapper.vm.prepareTask(sampleTask);

    // Now, you can make assertions about the component's task property
    const task = wrapper.vm.task;

    // Assert that the task has been updated correctly
    expect(task).to.deep.equal(sampleTask);
  });
