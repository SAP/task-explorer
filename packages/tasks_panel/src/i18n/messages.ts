export const messages = {
  LOGGER_NOT_AVAILABLE: (): string =>
    `Logs are not available for the Task Explorer Extension.`,
  ...
  GET_TREE_INTENTS: (intents: number): string =>
    `${intents} intents retrieved.`,
  GET_TREE_CHILDREN_BY_INTENT: (intent: string, tasks: number): string =>
    `${tasks} tasks retrieved for intent ${intent}.`,
  ACTIVATE_CONTRIB_EXT_ERROR: (extensionId: string): string =>
    `Could not activate the ${extensionId} extension.`,
  ...
  DELETE_TASK: (task: string): string =>
    `tasks-explorer.deleteTask command. got task: ${task}`,
  EDIT_TASK: (task: string): string =>
    `tasks-explorer.editTask command. got task: ${task}`,
  ...
  DUPLICATED_TYPE: (type: string): string =>
    `The type "${type}" is already contributed.`,
  EXECUTE_FAILURE: (task: string): string =>
    `Could not execute the task: ${task}`,
  EVALUATED_METHOD_FAILURE: (
    methodName: string,
    questionName: string,
    params: string,
    err: Error
